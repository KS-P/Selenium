...
public class DynamicWebtableWithPagination {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		System.out.println(text);//Showing 1 to 20 of 10131 (507 Pages)
				//getting 507 from the string(507 Pages)
	int total_pages=Integer.valueOf(text.substring(text.indexOf("(")+1,text.indexOf("P")-1));
System.out.println("Total pages are:" +total_pages);
		
//Find total number of rows in a page
for(int p=1;p<=5;p++)
{
...
int total_rows=driver.findElements(By.xpath("//table[@class='table table-bordered table-hover']//tbody/tr")).size();
 System.out.println("Total rows in a page:" +total_rows);
